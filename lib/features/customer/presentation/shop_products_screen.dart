  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<QuerySnapshot>(
        ...
        builder: (context, globalCategorySnapshot) {
          if (globalCategorySnapshot.connectionState ==
              ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }
          final globalCategories = globalCategorySnapshot.hasData
              ? globalCategorySnapshot.data!.docs
          ...
          return StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                ...
                .snapshots(),
            builder: (context, localCategorySnapshot) {
              if (localCategorySnapshot.connectionState ==
                  ConnectionState.waiting) {
                return const Center(child: CircularProgressIndicator());
              }

              final localCategories = localCategorySnapshot.hasData
                  ? localCategorySnapshot.data!.docs
                  : <QueryDocumentSnapshot>[];

              // Map for quick Category Name lookup
              final categoryMap = <String, String>{};
              for (var doc in globalCategories) {
                final data = doc.data() as Map<String, dynamic>;
                categoryMap[doc.id] = data['name'] ?? 'Unnamed Category';
              }
              for (var doc in localCategories) {
                final data = doc.data() as Map<String, dynamic>;
                categoryMap[doc.id] = data['name'] ?? 'Unnamed Category';
              }
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Column(
                        ...
                        children: [
                          ...
                          const SizedBox(
                              height: 16), // space between categories
                        ],
                      );
                    },
                  );
